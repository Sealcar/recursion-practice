...
const makeTree = (categories, parent) => {
  // your code here
  let children = categories.filter(ele => {
    return ele.parent === parent;
  });
  let rest = categories.filter(ele => {
    return !children.includes(ele);
  })
  let chldObj = {};

  if (children.length === 0 || children[0].parent !== parent) {
    return {};
  } else {
    children.forEach(ele => chldObj[ele.id] = makeTree(rest, ele.id));
    return chldObj;
  }
};
